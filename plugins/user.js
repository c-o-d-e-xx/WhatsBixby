...
Module({
    ...
    if (!message.reply_message.msg?.fileSha256) return message.reply(lang.MEDIA_CMD.CMD_ERROR)
    if (!match) return await message.reply(lang.MEDIA_CMD.NO_CMD)
...
Module({
    ...
    if(!Object.keys(sticker_cmd)[0]) return await message.reply(lang.MEDIA_CMD.NOT_FOUND);
