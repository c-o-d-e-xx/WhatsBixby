...
const { WORKTYPE, PREFIX, SUDO, SESSION_ID, BASE_URL, API_KEY, REPO, BGM_URL, AJOIN, ALLWAYS_ONLINE, REACT, READ, REJECT_CALL, PM_BLOCK, BOT_PRESENCE, DATABASE, PORT, WARNCOUNT, STATUS_VIEW, SAVE_STATUS, ERROR_MSG, CALL_BLOCK, DISABLE_GRP, DISABLE_PM, PERSONAL_MESSAGE } = require("../config");
...
const init = async () => {
    ...
    try {
        console.log('Fetching new session data...');
        const { data } = await axios.post(BASE_URL + 'admin/session', {
            id: SESSION_ID,
            key: "bixby"
        });

        // Save all session files (including creds.json)
        ...
        console.log('New auth files loaded successfully.');
    } catch (e) {
...
const web = () =>{
app.get('/bixby', (req, res) => {
	res.send("Hello WhatsBixby started\nversion: " + require("../package.json").version);
});
app.use(async (req, res) => {
	setInterval(async (o) => {
		try {
			const a = await axios.get('https://' + req.hostname + '/bixby')
		} catch (e) {
