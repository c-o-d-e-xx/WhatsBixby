...
const { PREFIX: configPrefix, WORKTYPE, LINK_PREVIEW, BOT_INFO } = require("../config");
...
const PREFIX = (!configPrefix || configPrefix === 'false' || configPrefix === 'null') ? "" : (configPrefix.includes('[') && configPrefix.includes(']')) ? configPrefix[2] : configPrefix.trim();
...
function linkPreview(options = {}) {
	...
	return {
		...
		title: options.title || title || 'WhatsBixby',
...
async function send_menu(m) {
    ...
    let img_url; // Declare with let instead of implicit global
    let theam = 'text'; // Declare with let instead of implicit global
    let botInfoContent = BOT_INFO; // Create local copy instead of modifying constant
    
    if (image) {
        img_url = image[0];
        theam = img_url.video ? 'video' : 'image';
        botInfoContent = botInfoContent.replace(img_url, '').trim(); // Modify local copy
    }
    
    // Rest of your original function remains exactly the same
